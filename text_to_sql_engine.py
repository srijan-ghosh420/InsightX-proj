...
def extract_sql(response_text):
    # This removes markdown blocks AND any leading 'SQL' label the AI might add
    sql_query = response_text.replace("```sql", "").replace("```sqlite", "").replace("```", "")
...
def generate_insight_from_data(user_query, db_results):
    """Pass 2: Reads the REAL database numbers and writes a text insight."""
    prompt = f"""
    The user asked the following question about their transaction data: "{user_query}"
    
    We ran the SQL query and the database returned exactly this data:
    {db_results}

    TASK:
        Analyze the database result and provide a highly structured, professional insight. 
        You MUST format your response exactly using the four bullet points below. Do not add intro or outro text.

        * **Direct Response:** [Answer the user's specific question directly in one sentence]
        * **Supporting Statistics:** [Cite the exact metrics from the database result]
        * **Contextual Insights:** [Explain the business pattern, trend, or context behind these numbers]
        * **Recommendation:** [Provide a brief, logical, and actionable next step for the business]

        STRICT CONSTRAINTS:
        - ALWAYS use the Indian Rupee symbol (₹) for monetary values. Never use '$'.
        - Base your statistics ONLY on the provided Database Result. Do not hallucinate data.
        """
